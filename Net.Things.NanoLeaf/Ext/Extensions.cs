...
namespace Ext
{
    ...
    {
        public static bool Try<T>(Func<T> fn, out T ret)
        {
            try { ret = fn(); return true; }
            catch { ret = default(T); return false; }
        }

        public static T Try<T>(Func<T> fn)
        {
            Try(fn, out var ret);
            return ret;
        }
    ...
    public static class Seq
    {
        ...
        public static IEnumerable<T> Fold<T>(IEnumerable<IEnumerable<T>> seqs)
            => seqs.SelectMany(x => x);
        //
        // Numerics
        //

        public static IEnumerable<int> From(int start = 0, int step = 1)
            => For(start, step: step);

        public static IEnumerable<int> To(int end, int step = 1)
            => For(end: end, step: step);

        public static IEnumerable<int> N()
            => For(0, step: 1);
        public static IEnumerable<int> N(int n, int step = 1)
            => For(0, n - 1, step);
        ...
        public static IEnumerable<T> Repeat<T>(this T value, int n)
         => Yield<T>(value);
        ...
        public static IEnumerable<T> ConcatSelf<T>(this IEnumerable<T> seq)
            => seq.Concat(seq);
    ...
    public static class Http
    {
        ...
        public async static Task<HttpResponseMessage> Put(this HttpClient c, string url, HttpContent content)
            => (await c.PutAsync(url, content));

        public async static Task<HttpResponseMessage> Put(this HttpClient c, string url, object content, string type = text_plain)
            => await c.Put(url, Content(content.ToString(), type));

        public async static Task<HttpResponseMessage> Put(this HttpClient c, string url, JToken json)
            => await c.Put(url, json, application_json);

        public static async Task<HttpResponseMessage> Post(this HttpClient c, string url, HttpContent content)
            => (await c.PostAsync(url, content));

        public static async Task<HttpResponseMessage> Post(this HttpClient c, string url, object content, string type = text_plain)
            => await  c.Post(url, Content(content.ToString(), type));

        public static async Task<HttpResponseMessage> Post(this HttpClient c, string url, JToken json)
            => await c.Post(url, json, application_json);
        ...
        public static async Task<string> Text(this Task<HttpContent> c)
            => await c.Result.ReadAsStringAsync();
        public static async Task<byte[]> Bytes(this Task<HttpContent> c)
            => await c.Result.ReadAsByteArrayAsync();
        public static async Task<Stream> Stream(this Task<HttpContent> c)
            => await c.Result.ReadAsStreamAsync();

        public static async Task<JsonReader> JsonReader(this Task<HttpContent> c)
            => new JsonTextReader(new StreamReader(await c.Stream()));

        public static async Task<JArray> JsonArray(this Task<HttpContent> c)
            => await c.Json<JArray>();
    ...
    public static partial class UriExt
    {
        static IPHostEntry GetHostEntry(string h)
            => Fn.Try(() => Dns.GetHostEntry(h));

        public static string GetCanonicalHost(string host)
            => GetHostEntry(host)?
                .AddressList
                .OrderBy(a => (a.AddressFamily, a.GetHashCode()))
                .FirstOrDefault()?
                .Apply(a => GetHostEntry(a.ToString()).HostName)
                ?? host;
